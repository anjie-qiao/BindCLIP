...
save_dir="savedir_finetune"
tmp_save_dir="tmp_save_dir_finetune"
tsb_dir="tsb_dir_finetune"
...
CUDA_VISIBLE_DEVICES="0"  $(which unicore-train) $data_path --user-dir ./unimol \
  --train-subset train --valid-subset valid \
  --num-workers 0 --ddp-backend=c10d \
  --task drugclip --loss in_batch_softmax --arch drugclip  \
  --max-pocket-atoms 256 \
  --optimizer adam --adam-betas "(0.9, 0.999)" --adam-eps 1e-8 --clip-norm 1.0 \
  --lr-scheduler polynomial_decay --lr $lr --warmup-ratio $warmup \
  --max-epoch $epoch \
  ...
  \
  --finetune-from-model checkpoint_best.pt \

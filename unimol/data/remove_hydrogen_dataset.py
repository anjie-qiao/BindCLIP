...
class DockRemoveHydrogenDataset(BaseWrapperDataset):
    def __init__(
        self,
        dataset,
        atoms,
    ...
    def __cached_item__(self, index: int, epoch: int):
        ...
        docked = dd[self.docked_coordinates]
        ...
        if not self.remove_hydrogen and self.remove_polar_hydrogen:
            ...
            if end_idx != 0:
                ...
                docked = docked[:-end_idx]
        dd[self.atoms] = atoms
        dd[self.coordinates] = coordinates.astype(np.float32)
        dd[self.docked_coordinates] = docked.astype(np.float32)
        return dd
